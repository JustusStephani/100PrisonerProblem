...
class PrisonExperiment:
    def __init__(
        self, numberOfPrisoners: int = 100, numberOfAttempts: int = 50
    ) -> None:
        """
    ...
    def singlePrisonerAttemptingDraws(
        self, prisoner: int, shelfOfDrawers: npt.NDArray
    ) -> int:
        """
    ...
    def performExperiment(
        ...
        # Each prisoner is drawing one after each other, "numberOFAttempts" times unit
        # he finds his number. If he does not find his number the experiment is unsuccessful
        for prisoner in prisoners:
            if self.singlePrisonerAttemptingDraws(prisoner, shelfOfDrawers) == 0:
                return 0
